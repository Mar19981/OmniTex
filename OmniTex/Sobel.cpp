...
void Sobel::apply(std::vector<QVector3D>& input, int startY, int width, int size, bool flipR, bool flipG)
{
	...
	while (size) {
		for (int x = 1; x < limitWidth; x++) {
			...
			input.at(startY * width + x).setX(rMul * (-topLeft - left - left - bottomLeft + topRight + right + right + bottomRight));
			input.at(startY * width + x).setY(gMul * (topLeft + top + top +  + topRight - bottomLeft - bottom - bottom - bottomRight));
			size--;
			if (!size) return;
		}
		startY++;
